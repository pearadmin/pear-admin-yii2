...
<input type="hidden" name="directSubmit" value="<?php echo isset($data['directSubmit'])?$data['directSubmit']:0; ?>">
...
layui.use('element', function(){
    ...
    $('.bs-glyphicons-list li').on('click', function () {
        $('.bs-glyphicons-list').find('li').each(function() {
           $(this).css('color','#000')
        ...
        var className = $(this).find('span.glyphicon,span.fa,i.layui-icon').get(0).className;
        if(!className){
            className = $(this).find('i.layui-icon').get(0).className;
        }
        if (className) {
            //console.log(className);
            //return false;
            //window.parent.document.getElementById('layui-layer1').contentWindow.document.getElementById('icon').value=className;
            window.parent.document.getElementById('icon').value = className;
            var index = parent.layer.getFrameIndex(window.name);
            if($('input[name="directSubmit"]').val() == 0){
